...
module.exports.savedUserAnswer = async (req, res) => {
    try {
        const { user_id, slot_id, answers } = req.body;
...
module.exports.getUserScoreForSlot = async (req, res) => {
    try {
        ...
        const userAnswers = await Answer.findOne({ user_id, slot_id });
        ...
        // Calculate the score
        const correctAnswers = userAnswers.answers.filter(answer => answer.is_correct);
